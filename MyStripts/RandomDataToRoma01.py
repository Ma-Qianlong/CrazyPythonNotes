...
def exec_many(datas, host='192.168.5.249', port=3136, user="root", password="root",
    ...
    cursor = conn.cursor()
    try:
        affRows = cursor.executemany('insert into cecep_czxl_hvc_opc_upload (id, dat, value) values (%s, %s, %s)', datas)
...
def exec_del(tagnameList, sTime, eTime, host='192.168.5.249', port=3136, user="root", password="root",
    ...
    try:
        ...
        affRows = cursor.executemany("delete from cecep_czxl_hvc_opc_upload where id = %s and dat between %s and %s ",
                                     argList)
